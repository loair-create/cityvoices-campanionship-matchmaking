...
Google Apps Script web apps do not send `Access-Control-Allow-Origin` for JSON responses, so a browser app on another domain (e.g. Lovable) cannot `fetch()` the script URL directly.
...
4. Save and deploy. Use the Worker’s `workers.dev` URL (or custom domain) as the **base URL** your Lovable app calls for API requests (same place you’d put the Apps Script URL if CORS allowed it).
