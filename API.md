...
2. **Deploy** the project as a **Web app** (new version after code changes).  
   ...
   - *Who has access:* Anyone (the **token** is the real auth).
3. Copy the **Web app URL** — that is your API base URL for POST requests.

**GET** is supported for simple reads:  
`{WEBAPP_URL}?api=1&action=getData&token=YOUR_TOKEN`

Browsers often **cannot** call the Google URL directly from JavaScript because of **CORS**. Use the included [cors-proxy](cors-proxy/) Worker or call the API from your **server** only.
